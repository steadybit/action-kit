...
func TestActionDescription(t *testing.T) {
	v := ActionDescription{
		...
		Kind:        Attack,
		TimeControl: TimeControlInternal,
		Hint: new(ActionHint{
			Type:    HintInfo,
		...
		Widgets: new([]Widget{
			StateOverTimeWidget{
				...
				Url: new(StateOverTimeWidgetUrlConfig{
					From: new("steadybit.url"),
				}),
				Value: new(StateOverTimeWidgetValueConfig{
					Hide: new(true),
				}),
			},
		}),
		Metrics: new(MetricsConfiguration{
			Query: new(MetricsQueryConfiguration{
		...
		Status: new(MutatingEndpointReferenceWithCallInterval{
			Method: POST,
			Path:   "/actions/rollout-restart/status",
		}),
		Stop: new(MutatingEndpointReference{
			POST,

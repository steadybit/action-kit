...
func Test_getMask(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "any port",
			portRange: network.PortRangeAny,
			want:      []string{"0 0x0000"},
		},
		{
			name:      "single port",
			portRange: network.PortRange{From: 80, To: 80},
			want:      []string{"80 0xffff"},
		},
		{
			name:      "range",
			portRange: network.PortRange{From: 1000, To: 1999},
			want:      []string{"1000 0xfff8", "1008 0xfff0", "1024 0xfe00", "1536 0xff00", "1792 0xff80", "1920 0xffc0", "1984 0xfff0"},
		},
		{
			name:      "small range",
			portRange: network.PortRange{From: 8080, To: 8081},

...
func necessaryExcludes(excludes []network.NetWithPortRange, includes []network.NetWithPortRange) []network.NetWithPortRange {
	result := make([]network.NetWithPortRange, 0, len(excludes))
...
func tcCommandsForNets(netWithPortRanges []network.NetWithPortRange, mode mode, ifc, parent, flowId string, prio int) ([]string, error) {
	var cmds []string
...
func getMatchers(nwp network.NetWithPortRange) ([]string, error) {
...
func deduplicateNetWithPortRange(nwps []network.NetWithPortRange) []network.NetWithPortRange {
	slices.SortFunc(nwps, network.NetWithPortRange.Compare)

	var deduplicated []network.NetWithPortRange
...
func mustParseNetWithPortRange(netStr, port string) network.NetWithPortRange {
	parsedNet, err := network.ParseCIDR(netStr)
	if err != nil {
		panic(err)
	}
	parsedPort, err := network.ParsePortRange(port)
	if err != nil {
		panic(err)
	}
	return network.NetWithPortRange{Net: *parsedNet, PortRange: parsedPort}
}

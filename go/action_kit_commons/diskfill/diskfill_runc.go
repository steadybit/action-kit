...
func CheckPathWritableRunc(ctx context.Context, r ociruntime.OciRuntime, sidecar SidecarOpts, targetPath string) error {
	bundle, err := createBundle(ctx, r, sidecar, targetPath,
		"sh", "-c", "test -d "+mountpointInContainer+" && touch "+mountpointInContainer+"/.steadybit-diskfill-check && rm -f "+mountpointInContainer+"/.steadybit-diskfill-check")
	if err != nil {

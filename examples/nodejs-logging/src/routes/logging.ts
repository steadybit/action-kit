// SPDX-License-Identifier: MIT
// SPDX-FileCopyrightText: 2022 Steadybit GmbH

import {
	DescribeActionResponse,
	PrepareActionResponse,
	StartActionResponse,
	ActionStatusResponse,
	StopActionResponse,
} from '@steadybit/action-kit-api';
import express from 'express';

export const router = express.Router();

router.get('/actions/logging', (_, res) => {
	const response: DescribeActionResponse = {
		id: 'com.github.steadybit.example.logging',
		label: 'Logging Action',
		icon: 'data:image/svg+xml;base64,',
		description: 'Prints the received payload to the console to illustrate the action API.',
		version: String(Date.now()),
		category: 'resource',
		kind: 'other',
		targetType: 'cat',
		timeControl: 'internal',

		parameters: [
			{
				name: 'text',
				label: 'Text',
				type: 'string',
				required: true,
			},
			{
				name: 'level',
				label: 'Level',
				type: 'string',
				required: true,
				options: [
					{ label: 'Info', value: 'info' },
					{ label: 'Warn', value: 'warn' },
					{ label: 'Error', value: 'error' },
				],
			},
			{
				name: 'env',
				label: 'Environment',
				type: 'key_value',
			},
		],

		prepare: {
			method: 'post',
			path: '/actions/logging/prepare',
		},
		start: {
			method: 'post',
			path: '/actions/logging/start',
		},
		status: {
			method: 'post',
			path: '/actions/logging/status',
		},
		stop: {
			method: 'post',
			path: '/actions/logging/stop',
		},
	};
	res.json(response);
});

router.post('/actions/logging/prepare', (req, res) => {
	console.log('Got prepare request:', JSON.stringify(req.body));

	const response: PrepareActionResponse = {
		state: {
			generatedBy: 'prepare',
			text: req.body.config.text,
		},
		messages: [{ message: 'Prepared the action!' }],
	};
	res.json(response);
});

router.post('/actions/logging/start', (req, res) => {
	console.log('Got start request:', JSON.stringify(req.body));

	const response: StartActionResponse = {
		state: {
			...req.body.state,
			generatedBy: 'start',
		},
		messages: [{ message: 'Started the action!' }],
	};
	res.json(response);
});

router.post('/actions/logging/status', (req, res) => {
	console.log('Got status request:', JSON.stringify(req.body));

	const response: ActionStatusResponse = {
		completed: true,
		state: {
			...req.body.state,
			generatedBy: 'status',
		},
		messages: [{ message: 'Check the action status!' }],
	};
	res.json(response);
});

router.post('/actions/logging/stop', (req, res) => {
	console.log('Got stop request:', JSON.stringify(req.body));
	const response: StopActionResponse = {
		messages: [{ message: 'Stopped the action!' }],
	};
	res.json(response);
});
